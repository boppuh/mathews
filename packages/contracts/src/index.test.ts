...
describe("task cockpit vocabularies", () => {
  it("keeps workflow boundaries explicit", () => {
    ...
    expect(TASK_EVIDENCE_STATUSES).toEqual(["AVAILABLE", "CORRECTION", "SUPERSEDED", "DELETED"]);
    expect(TASK_EVIDENCE_CATEGORIES).toContain("PR_CI");
    expect(TASK_EVIDENCE_CONTENT_ACCESS).toEqual([
      "AVAILABLE",
