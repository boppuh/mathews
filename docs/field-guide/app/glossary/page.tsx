...
export default function GlossaryPage() {
  ...
  return <main className="doc-shell glossary-shell"><DocNav current="glossary" />
    ...
    <section className="glossary-tools" aria-label="Glossary filters"><label><span>SEARCH TERMS</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try “exact head”, “lease”, or “approval”…" /></label><div role="group" aria-label="Glossary categories">{categories.map((item) => <button key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)} aria-pressed={category === item}>{item}</button>)}</div><p><strong>{visible.length}</strong> of {terms.length} terms</p></section>
    <section className="glossary-list" aria-live="polite">{visible.length ? visible.map(([term, group, definition], index) => <article key={term}><span>{String(index + 1).padStart(2,"0")}</span><div><small>{group}</small><h2>{term}</h2></div><p>{definition}</p></article>) : <div className="glossary-empty"><span>NO MATCH</span><p>Try a broader term or select All.</p></div>}</section>
